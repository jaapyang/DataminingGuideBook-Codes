...
class TestFilteringData(unittest.TestCase):
    def test_calculate_nearest_neighbor(self):
        distances = calculate_nearest_neighbor("Bill", users)
        expected = [4.0, 5.0, 5.5, 6.0, 8.0, 9.0, 14.0]
        self.assertEqual(distances, expected)

    def test_manhattan(self):
        distance = manhattan(users['Sam'], users["Bill"])
        expected = 8.0
